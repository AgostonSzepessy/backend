...
export class ChatParticipation {
    ...
    public static async add(chat_id: number, username: string) {
        ...
        const chat_participation_id =
          (await knex('ChatParticipation').insert(data).returning('chat_participation_id'))[0];
    ...
    public static async getChatsForUser(username: string) {
      return knex('ChatParticipation')
        ...
        .innerJoin('Chat', 'ChatParticipation.chat_id', '=', 'Chat.chat_id');
    }
